    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById("salir").addEventListener('click', function(event) {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = '{{ route('sala.global') }}';
                    }
                });
            ...
            function subirDatosUno(tiempoTotal, porcentajeEfectividad, miDificultad, tema, acerto, fallo, salaId) {
                var estadisticasGuardarUrl = "{{ route('estadisticas.guardarGlobal') }}";
            ...
            function verificarLogro(tema) {
                fetch('/verificar-logro', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                    ...
                    .then(data => {
                        if (data.success) {
                            // Logro desbloqueado, muestra el nombre y la imagen
                            mostrarLogro(data.logro_nombre, data.logro_imagen, data.tema_logro);
                        } else {
                            // No se desbloqueó ningún logro, muestra mensaje de error o no hacer nada
            ...
            function mostrarLogro(nombre, imagen, tm) {
                // Aquí puedes implementar cómo deseas mostrar el logro en tu interfaz
                console.log('¡Logro desbloqueado!');
                Swal.fire({
                    title: "¡Logro desbloqueado!",
                    text: `Acabas de obtener el logro ${nombre}. 
    \n Por haber conseguido el 100% de efectividad en ${(tm == 1)? "Acentuación" : (tm==2)? "Uso de letras" : (tm==3)? "Concordancia" : (tm==4)? "Gramatica General" : "todos los temas"}.`,
                    imageUrl: "{{ asset('/') }}" + imagen,
                    confirmButtonText: "De acuerdo",
                    imageWidth: 200,
                    imageHeight: 200,
                    imageAlt: `${nombre} Image`
                }).then(() => {
                    verificarLogroFinal()
                    // window.location.href = "{{ route('sala.global') }}"
        ...
        function verificarLogroFinal() {

            $.post('/verificar-logro-final', {
                _token: "{{ csrf_token() }}"
            }, function(data) {
                if (data.logro) {
                    // Mostrar el logro desbloqueado
                    mostrarLogro(data.logro.nombre, data.logro.imagen);
                } else {
                    window.location.href = "{{ route('sala.global') }}"
                }
            });
        ...
        function mostrarLogro(nombre, imagen) {
            console.log('¡Logro desbloqueado!');
            Swal.fire({
                title: "¡Logro desbloqueado!",
                text: `Acabas de obtener el logro Sabio Ortográfico: Alcanza el 100% de efectividad en todos los temas.`,
                confirmButtonText: "De acuerdo",
                imageUrl: "{{ asset('assets/img/logros/logro-final.webp') }}",
