...
@section('nav')
    <a href="/salas/privada/redirect/{{ $sala_id }}"
        class="nav-link nav-profile d-flex align-items-center pe-0">Regresar</a>
@endsection
@section('name-sala')
    <div class="pagetitle">
        <h1>{{ $nombre_sala }}</h1>
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{ $codigo_sala }}</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
@endsection
...
@php
    ...
    $sala = \App\Models\Salas::find($sala_id);
    // Suponiendo que `dificultades` y `temas` están almacenados como arrays en la tabla `salas`
    $dificultades2 = json_decode($sala->dificultades, true);
    // Si el array de dificultades está vacío, asignar las dificultades predeterminadas
    if (empty($dificultades2)) {
        $dificultades2 = ['facil', 'medio', 'dificil'];
    }
@endphp
@section('dificut')
    @foreach ($dificultades2 as $dificultad)
        <th>{{ ucfirst($dificultad) }}</th>
    @endforeach
@endsection
        ...
        @foreach ($estadisticasDetalladas as $usuario => $temas)
            @foreach ($temas as $tema => $dificultades)
                <tr>
                    ...
                    @foreach ($dificultades2 as $dificultad)
                        <td>
                            {{ isset($dificultades[$dificultad]) ? $dificultades[$dificultad] . '%' : '0%' }}
                        </td>
                    @endforeach
                </tr>
            @endforeach
